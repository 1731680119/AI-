...
def generate_title(settings: dict, model: str, first_user_msg: str) -> str:
    """根据首条消息生成简短标题"""
    client = build_client(settings)
    try:
        ...
        # 去掉可能混入的 think 标签
        title = THINK_CLOSE.split(THINK_OPEN.split(title)[-1])[-1].strip()
        return title[:30] or "新对话"
    except Exception as error:
        # 标题生成失败不影响对话，降级为 WARNING 但仍留下堆栈。
        diag.log_event(
            "WARNING", "backend", f"标题生成失败，回退为截断首句：{error}",
            logger="backend.llm", model=model, error_type=type(error).__name__,
        )
